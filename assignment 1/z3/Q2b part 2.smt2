...
(assert (T5 a0 a1 a2 b0 b1 b2)) 
(check-sat)
(get-model)

;give b0=false, b1=false, b2=false. That is , x=0

(assert (not (and (not b0)(not b1) (not b2))))
(check-sat)
(get-model)

;give b0=true, b1=true, b2=true. That is , x=7
...
;So x=0,7 are the only fixed points reachable after 5 iterations.